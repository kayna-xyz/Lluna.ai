...
export async function GET(req: Request) {
  ...
  if (!supabase) {
    return Response.json({ settings: null, configured: false })
  }
  ...
  const { data, error } = await supabase
    ...
    .select('payload')
    .eq('event_type', 'campaign_settings_updated')
  ...
  return Response.json({
    settings: data?.payload ? asRec(data.payload) : null,
    configured: true,
  })
}
...
export async function POST(req: Request) {
  ...
  const { error } = await supabase.from('consultant_events').insert({
    ...
    event_type: 'campaign_settings_updated',
